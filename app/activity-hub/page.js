...
import {
  Plus, Camera, Video, AlertTriangle, Briefcase, DollarSign, Heart, Building2,
...
const FILTERS = [
  ...
  { key: 'bounties',   label: 'Bounties',    icon: DollarSign,    tone: 'text-green-600 bg-green-100' },
...
const POST_TYPES = [
  ...
  { value: 'bounty',           label: 'Bounty',           icon: DollarSign,    tone: 'text-green-600 bg-green-50',       desc: 'Start a community bounty', href: '/bounties/new' },
...
const FEED_TYPE_META = {
  ...
  bounty:             { label: 'BOUNTY',          tone: 'bg-green-100 text-green-800',     icon: DollarSign },
...
function ActivityHubInner() {
  ...
  return (
    <PageShell active="activity" maxWidth="max-w-6xl" bg="bg-neutral-50"
        ...
        <CardContent className="flex items-center gap-3 p-3">
          ...
          <div className="flex gap-1.5">
            {[
              ...
              { Icon: DollarSign,    tone: 'bg-green-100 text-green-600' },
            ].map(({ Icon, tone }, i) => (
      ...
      <CardContent className="p-0">
          ...
          <div className="flex items-center justify-between gap-2 px-3 pt-3">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <div className="truncate text-sm font-bold text-neutral-900">{card.author?.name || 'User'}</div>
                <div className="text-[10px] uppercase tracking-wider text-neutral-500">{timeAgoShort(card.createdAt)}</div>
              </div>
            </div>
            <span className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${meta.tone}`}>
        ...
        {!isUserPost && (
          <div className="flex items-center justify-between gap-2 px-3 pt-3">
            ...
            <span className="text-[10px] text-neutral-400">{timeAgoShort(card.createdAt)}</span>
          </div>
        )}

        {/* Body */}
        <div className="px-3 pb-3 pt-2">
          {card.title && <h3 className="text-base font-extrabold leading-snug text-neutral-900">{card.title}</h3>}
          {isJob && card.budget != null && (
            <div className="mt-1 text-2xl font-extrabold tracking-tight text-green-700">${Number(card.budget).toLocaleString()}</div>
          )}
          {isBounty && (
            <div className="mt-2 rounded-lg border border-green-200 bg-green-50 p-2.5">
              ...
              <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-green-200">
          ...
          {card.description && <p className="mt-1.5 line-clamp-3 text-sm text-neutral-700">{card.description}</p>}
          {(card.location?.city || card.city) && (
            <div className="mt-1.5 inline-flex items-center gap-1 text-xs text-neutral-500">
              <MapPin className="h-3 w-3" /> {[card.location?.city || card.city, card.location?.state || card.state].filter(Boolean).join(', ')}
            </div>
          )}
        </div>
        ...
        {/* Image */}
        {cover && (
          <Link href={card.href || '#'} className="block bg-neutral-100">
            <SafeImage src={cover} alt="" className="max-h-72 w-full object-cover" />
          </Link>
        )}
        ...
        {isUserPost ? (
          <>
            <div className="flex items-center justify-between gap-1 border-t border-neutral-100 px-2 py-1.5 text-xs">
              <div className="flex items-center gap-0.5">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-md px-2.5 py-1.5 font-medium transition hover:bg-neutral-100 ${commentsOpen ? 'text-brand-700' : 'text-neutral-600'}`}
                >
                ...
                <span className="ml-1 inline-flex items-center gap-1 px-1.5 text-[10px] text-neutral-400">
                  <Eye className="h-3 w-3" /> {card.views || 0}
                </span>
              </div>
              {card.href && (
                <Link href={card.href} className="rounded-md px-2.5 py-1.5 text-xs font-bold text-brand-700 hover:bg-brand-50">
                  View →
                </Link>
              )}
            </div>
          ...
          <div className="flex items-center justify-between gap-2 border-t border-neutral-100 px-3 py-2 text-xs text-neutral-500">
            ...
            {card.href && (
              <Link href={card.href} className="inline-flex items-center gap-1 rounded-md bg-green-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-green-800">
...
function ActionButton({ onClick, active, icon: Icon, label, activeTone }) {
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1 rounded-md px-2.5 py-1.5 font-medium transition ${active ? activeTone : 'text-neutral-600'} hover:bg-neutral-100`}
    >
...
function AuthorAvatar({ author }) {
  ...
  return (
    <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-brand-100 text-sm font-bold text-brand-700">
      {url ? <SafeImage src={url} alt="" className="h-full w-full object-cover" /> : initials}
    </div>
  )
}
