...
import { ShoppingBag, MapPin, MessageCircle, Bookmark, BadgeCheck, AlertTriangle, Loader2, Gift, Clock, Timer, ShieldCheck, X, CheckCircle2, Truck } from 'lucide-react'
...
export default function MarketplaceListingDetailPage() {
  ...
  if (loading) return <FieldFrame title="Listing" back="/"><div className="py-10 text-center text-sm text-neutral-400">Loading…</div></FieldFrame>
  if (err || !listing) return <FieldFrame title="Listing" back="/"><div className="px-4 py-10 text-center text-sm text-neutral-500">{err || 'Listing not found.'}</div></FieldFrame>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setReserveOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl bg-neutral-900 p-6 text-white shadow-2xl">
            ...
            <ul className="mt-4 space-y-2 text-sm">
              <li className="flex items-start gap-2"><span className="text-emerald-400">✓</span> Item will be held for you</li>
              <li className="flex items-start gap-2"><span className="text-emerald-400">✓</span> Seller will share exact location</li>
              <li className="flex items-start gap-2"><span className="text-emerald-400">✓</span> First come, first served</li>
            </ul>
