    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl overflow-hidden p-0">
        <div className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr]">
          {/* ───── LEFT: photo carousel ───── */}
          <div className="relative aspect-[4/3] bg-neutral-900 md:aspect-auto md:min-h-[420px]">
            {hasPhotos ? (
              <img
                src={photos[idx]}
                alt={listing.title}
          ...
          {/* ───── RIGHT: details ───── */}
          <div className="flex flex-col p-5 md:max-h-[600px] md:overflow-y-auto">
            <div className="flex-1">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <h3 className="line-clamp-2 text-lg font-extrabold leading-tight">{listing.title}</h3>
                  <div className={`mt-1 text-2xl font-black ${listing.priceType === 'free' || listing.price === 0 ? 'text-emerald-600' : 'text-neutral-900'}`}>
                    {priceStr}
                  </div>
                </div>
                <button
              ...
              {/* Description */}
              {listing.description && (
                <p className="mt-4 line-clamp-6 whitespace-pre-wrap text-[13.5px] leading-relaxed text-neutral-700">
                  {listing.description}
                </p>
              )}
            ...
            <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              >
                ...
                {viewerIsOwner ? 'This is your listing' : 'Contact Seller'}
              </Button>
              <Button onClick={goFull} variant="outline" className="h-11 border-emerald-200 font-semibold text-emerald-700 hover:bg-emerald-50">
                <ExternalLink className="mr-1.5 h-4 w-4" /> View Full Details
              </Button>
            </div>
          </div>
        </div>
